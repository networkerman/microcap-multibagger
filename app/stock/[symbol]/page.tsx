...
export default function StockPage() {
  ...
  const fetchReport = async (setLoadingState = true) => {
    if (setLoadingState) setStatus("loading");
    ...
    if (!data) {
      ...
      setStatus("analyzing");
      setTimeout(() => fetchReport(false), 5000);
    } else {
      setReport(data);
  ...
  useEffect(() => { fetchReport(); }, [symbol, exchange]);
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        {status === "analyzing" && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <div style={{ color: "#3d5a73", fontSize: 13, marginBottom: 32 }}>
              Claude is scoring 12 signals using public filings, screener.in data, exchange filings…
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
              {["S1","S2","S3","S4","S5","S6","S7","S8","S9","S10","S11","S12"].map(id => (
                <div key={id} style={{
                  width: 40, height: 40, borderRadius: 8,
                  background: ["S3","S4","S10"].includes(id) ? "#0f1f35" : "#0c1d2c",
                  border: `1px solid ${["S3","S4","S10"].includes(id) ? "#f59e0b44" : "#1a2e40"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: ["S3","S4","S10"].includes(id) ? "#f59e0b" : "#3d5a73",
            ...
            <p style={{ color: "#2e4a60", fontSize: 12, marginTop: 24 }}>We'll email you when it's ready. This page auto-refreshes.</p>
          </div>
        )}
        ...
        {status === "has-report" && report && (
          <>
            {/* Refresh overlay — slides in on top of the report */}
            {showRefreshOverlay && (
              <div style={{
